...
class Tips
{
	...
	private function indexAction()
	{
		...
		$this->consulta = "select * from tips";
        ...
        $this->consulta = "select * from tips limit ?, ?";
        $this->pstmt = $this->conexion->prepare($this->consulta);
        ...
        foreach ($this->pstmt->fetchAll(PDO::FETCH_ASSOC) as $row) 
        {
        	$this->result .= '<tr>
                                <td>'.$row['si_folio'].'</td>
                                <td>'.$row['si_t_name'].'</td>
                                <td>'.nl2br($row['si_description']).'</td>
                                <td class="actions">
                                    <a href="#" class="btn btn-sm btn-warning" onclick="editTip('.$row['si_folio'].')"><i class="glyphicon glyphicon-pencil"></i></a>
                                    <a href="#" class="btn btn-sm btn-danger" onclick="deleteTip('.$row['si_folio'].')"><i class="glyphicon glyphicon-trash"></i></a>
                                </td>
                            </tr>';
        }
        ...
        if($countProducts > $page_size)
        {
                ...
                if(($page - 1) > 0){
                    $this->result .= '<li><a href="#" onclick="paginaTips('.($page-1).')"><b>Previous</b></a></li>';
                }
                ...
                for($i=1; $i <= $total_paginas; $i++){
                    if($page == $i){
                        ...
                        $this->result .= '<li><a href="#" onclick="paginaTips('.$i.')">'.$i.'</a></li>';
                    }
                }

                if($page < $total_paginas){
                    $this->result .= '<li><a href="#" onclick="paginaTips('.($page+1).')"><b>Next</b></a></li>';
                }
                $this->result .= '</ul>
                </div>';
        }
    ...
    {
        $name = htmlspecialchars($_POST['tipname']);
        $description = htmlspecialchars($_POST['description']);

        try {

            $this->consulta = "insert into tips values(null, ?,?)";
            $this->pstmt = $this->conexion->prepare($this->consulta);
            $this->pstmt->bindParam(1, $name, PDO::PARAM_STR);
            $this->pstmt->bindParam(2, $description, PDO::PARAM_STR);
    ...
    private function editAction()
    {
        ...
        try 
        {
            $this->consulta = "select * from tips where si_folio = ?";
            $this->pstmt = $this->conexion->prepare($this->consulta);
    ...
    private function updateAction()
    {
        try {
            ...
            $this->consulta = "update tips set si_t_name = ? , si_description = ? where si_folio = ? ";
            $this->pstmt = $this->conexion->prepare($this->consulta);
            $this->pstmt->bindParam(1, $name, PDO::PARAM_STR);
            $this->pstmt->bindParam(2, $description, PDO::PARAM_STR);
    ...
    private function deleteAction()
    {
        ...
        try {
            ...
            $this->consulta = "delete from tips where si_folio = ?";
            $this->pstmt = $this->conexion->prepare($this->consulta);
